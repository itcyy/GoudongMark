      ...
      <div class="nav-login">
        <ul class="nav-login-item">
          <li class="nav-item" v-for="(item, index) in loginData" :key="index">
            ...
            <template v-else>
              <router-link :to="item.url" target="_blank" v-if="name==''">{{item.value}}</router-link>
...
export default {
  data () {
    return {
      ...
      loginData: [
        {value: '登陆', url: {path:'/Login',query:{"seesion":"0"}}},
        {value: '注册', url: { path: '/Login', query:{"seesion": "1"}}},
    ...
    console.log(ss,455555)
    if (ss == "01") {
      this.loginData = [
        { value: '个人中心', url: { path: '/user/admin' } },
        { value: '消息通知', url: 'http://order.mi.com/message/list' }
      ]
    }
  },
